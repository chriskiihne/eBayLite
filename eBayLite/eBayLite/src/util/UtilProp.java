...
public class UtilProp {
   ...
   static String _PROP_FILENAME_WIN_LOCAL = UtilProp.class.getProtectionDomain().getCodeSource().getLocation().toExternalForm().replace("file:/", "").replace("WEB-INF/classes/", "config.properties").replace("%20", " ").replace('/', '\\');
   static String _PROP_FILENAME_OSX_LOCAL = "/YOUR_PATH/webproject/WebContent/config.properties";
   // * Remote server path
   static String _PROP_FILENAME_REMOTE = "/var/lib/tomcat7/webapps/eBayLite/config.properties";
   static Properties prop = new Properties();

   static void loadProperty() throws Exception {
